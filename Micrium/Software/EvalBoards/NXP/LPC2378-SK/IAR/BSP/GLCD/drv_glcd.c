...
static const GLCD_CmdCtrl_t GLCD_Cmd_Iss0[GLCD_CMD_NUMB] = {
    ...
    {0x38, GLCD_WRITE, 0, 0},           // IDMOFF           - Idle mode off
    {0x39, GLCD_WRITE, 0, 0},           // IDMON            - Idle mode on
    {0xDA, GLCD_READ,  0, 1},           // RDID1            - Interface pixel format
    {0xDB, GLCD_READ,  0, 1},           // RDID2            - Interface pixel format
    {0xDC, GLCD_READ,  0, 1},           // RDID3            - Interface pixel format

    {0xB4, GLCD_WRITE, 0, 4},           // FRMSEL           - External oscillator select
    ...
    {0xB5, GLCD_WRITE, 0, 4},           // FRM8SEL          - External oscillator select
                                        // Frame frequency in Temp range A
                                        // Frame frequency in Temp range B
                                        // Frame frequency in Temp range C
                                        // Frame frequency in Temp range D

    {0xB6, GLCD_WRITE, 0, 4},           // TMPRNG           - External oscillator select
                                        // Temp range A
                                        // Temp range B
                                        // Temp range C
                                        // Temp range D

    {0xB8, GLCD_WRITE, 0, 1},           // TMPHIS           - Temp range set
    {0xB7, GLCD_READ,  0, 1},           // TMPREAD          - Temperature read back
    {0xD1, GLCD_WRITE, 0, 0},           // EPWRIN           - EEPROM write start
    {0xD0, GLCD_WRITE, 0, 0},           // EPWROUT          - EEPROM write end
    {0xD4, GLCD_WRITE, 0, 1},           // RDEV             - Read internal contrast (EV_IN)
    {0xD5, GLCD_READ,  0, 1},           // RDRR             - Read internal resistor ratio (RR_IN)

                                        // Adapted commands
    {0x00, GLCD_WRITE, 0, 0},           // PTIN             - adapted command (PTLAR + PTIN)

    {0x33, GLCD_WRITE, 0, 3},           // SCRLAR           - Scroll area set (2-line unit)
                                        // TFA = 0,1,2, … , 132
                                        // VSA = 0,1,2, … , 132
                                        // BFA = 0,1,2, … , 132

    {0x00, GLCD_WRITE, 0, 0},           // DATCTR           - adapted command (MADCTR + COLMOD)
    {0x25, GLCD_WRITE, 0, 1},           // SETCON           - Write contrast 0 - 127

                                        // Commands applicable only when ISS=1
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // OSCON
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // OSCOFF
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // RMWIN
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // RMWOUT
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // VOLUP
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // VOLDOWN
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // ASCSET
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // DISCTR
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // PWRCTR
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // VOLCTR

                                        // Commands applicable only when ISS=0
    {0xB0, GLCD_WRITE, 0, 0},           // CLKINT           - Internal oscillator select
    {0xB1, GLCD_WRITE, 0, 0},           // CLKEXT           - External oscillator select
    {0x09, GLCD_READ,  1, 4},           // RDDST            - Read Display Status
    {0x01, GLCD_WRITE, 0, 0},           // SWRESET          - Software reset
    {0x04, GLCD_READ,  1, 3},           // RDDID            - Read Display ID

    {0x30, GLCD_WRITE, 0, 2},           // PTLAR            - Partial start/end address set
                                        // Start address (0,1,2, …, 131)
                                        // End   address (0,1,2, …, 131)

    {0x12, GLCD_WRITE, 0, 0},           // PTLON            - Partial mode on
    {0x36, GLCD_WRITE, 0, 1},           // MADCTR           - Tearing effect mode set & on

    {0x3A, GLCD_WRITE, 0, 1},           // COLMOD           - Interface pixel format
                                        // Interface format

    {0x22, GLCD_WRITE, 0, 0},           // APOFF            - All pixel off (only for test purpose)
    {0x23, GLCD_WRITE, 0, 0},           // APON             - All pixel on (only for test purpose)
    {0x02, GLCD_WRITE, 0, 0},           // BSTROFF          - Booster off (only for test purpose)
    {0x03, GLCD_WRITE, 0, 0},           // BSTRON           - Booster on (only for test purpose)
};

static const GLCD_CmdCtrl_t GLCD_Cmd_Iss1[GLCD_CMD_NUMB] =
{
                                        // (ISS = 1)
    {0x25, GLCD_WRITE, 0, 0},           // NOP              - No Operation
    {0x95, GLCD_WRITE, 0, 0},           // SLPIN            - Sleep in & booster off
    {0x94, GLCD_WRITE, 0, 0},           // SLPOUT           - Sleep out & booster on
    {0xA9, GLCD_WRITE, 0, 0},           // PTLOUT (NORON)   - Partial off (Normal)
    {0xA6, GLCD_WRITE, 0, 0},           // INVOFF           - Display inversion off (normal)
    {0xA7, GLCD_WRITE, 0, 0},           // INVON            - Display inversion on
    {0xAE, GLCD_WRITE, 0, 0},           // DISPOFF          - Display off
    {0xAF, GLCD_WRITE, 0, 0},           // DISPON           - Display on

    {0x15, GLCD_WRITE, 0, 2},           // CASET            - Column address set
                                        // X_ADR start: 0h <= XS <= 83h
                                        // X_ADR end  : 0h <= XS <= 83h

    {0x75, GLCD_WRITE, 0, 2},           // RASET            - Row address set
                                        // Y_ADR start: 0h <= YS <= 83h
                                        // Y_ADR end  : 0h <= YS <= 83h

    {0x5C, GLCD_WRITE, 0, (Int32U)-1},  // RAMWR            - Memory write

    {0xCE, GLCD_WRITE, 0, 20},          // RGBSET           - Color set for 256 color display

    {0xAB, GLCD_WRITE, 0, 1},           // VSCSAD           - Scroll start address of RAM
                                        // SSA = 0, 1, 2, … , 131
    ...
    {0xB8, GLCD_WRITE, 0, 1},           // TMPHIS           - Temp hysteresis range set
    {0xB7, GLCD_READ,  0, 1},           // TMPREAD          - Temperature read back
    {0xCD, GLCD_WRITE, 0, 0},           // EPWRIN           - EEPROM write start
    {0xCC, GLCD_WRITE, 0, 0},           // EPWROUT          - EEPROM write end
    {0x7C, GLCD_WRITE, 0, 1},           // RDEV             - Read internal contrast (EV_IN)
    {0x7D, GLCD_READ,  0, 1},           // RDRR             - Read internal resistor ratio (RR_IN)

                                        // Adapted commands
    {0xA8, GLCD_WRITE, 0, 2},           // PTIN             - Partial area set & on
                                        // PSL7-0 Partial start line address
                                        // PEL7-0 Partial end line address

    {0xAA, GLCD_WRITE, 0, 4},           // SCRLAR(ASCSET)   - Scroll area set (2-line unit) Mode-0
                                        // SSL7-0 - Scroll start line
                                        // SEL7-0 - Scroll end line
                                        // SFL7-0 - Scroll specified line
                                        // SMD2-0 - Area scroll mode

    {0xBC, GLCD_WRITE, 0, 3},           // DATCTR           - adapted command (MADCTR + COLMOD)
                                        // MV MX MY	    = Row/column address control
                                        // MRGB         = RGB
                                        // GS2 GS1 GS0	= Gray-scale setup

    {0x00, GLCD_WRITE, 0, 1},           // SETCON           - Write contrast 0 - 127

                                        // Commands applicable only when ISS=1
    {0xD1, GLCD_WRITE, 0, 0},           // OSCON            - Oscillator on
    {0xD2, GLCD_WRITE, 0, 0},           // OSCOFF           - Oscillator off
    {0xE0, GLCD_WRITE, 0, 0},           // RMWIN            - Read & modify write on
    {0xEE, GLCD_WRITE, 0, 0},           // RMWOUT           - Read & modify write off
    {0xD6, GLCD_WRITE, 0, 0},           // VOLUP            - EV increment
    {0xD7, GLCD_WRITE, 0, 0},           // VOLDOWN          - EV decrement

    {0xAA, GLCD_WRITE, 0, 4},           // ASCSET           - Scroll area set
                                        // SSL7-0 - Scroll start line
                                        // SEL7-0 - Scroll end line
                                        // SFL7-0 - Scroll specified line
                                        // SMD1-0 - Area scroll mode

    {0xCA, GLCD_WRITE, 0, 3},           // DISCTR           - Display control
                                        // P1
                                        // P2
                                        // P3

    {0x20, GLCD_WRITE, 0, 1},           // PWRCTR           - Power control set
                                        // P13-P10

    {0x81, GLCD_WRITE, 0, 2},           // VOLCTR

                                        // Commands applicable only when ISS=0
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // CLKINT           - Internal oscillator select
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // CLKEXT           - External oscillator select
    {GLCD_INV_CMD, GLCD_READ,  0, 0},   // RDDST            - Read Display Status
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // SWRESET          - Software reset
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // RDDID            - Read Display ID
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // PTLAR            - Partial start/end address set
                                        // Start address (0,1,2, …, 131)
                                        // End address (0,1,2, …, 131)

    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // PTLON            - Partial mode on
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // MADCTR           - Tearing effect mode set & on

                                        // Interface format
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // COLMOD           - Interface pixel format
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // APOFF            - All pixel off (only for test purpose)
    ...
    {GLCD_INV_CMD, GLCD_WRITE, 0, 0},   // BSTRON           - Booster on (only for test purpose)
...
GLCD_Status_t  GLCD_SendCmd (GLCD_Cmd_t Cmd, pInt8U pData, Int32U Size)
{
    ...
    static  Int32U           ResRatio;                          // Hold resistor ratio init by VOLCTR and used by SETCON adapted command

            pGLCD_CmdCtrl_t  pGLCD_CmdCtrl;                     // Pointer to current set of commands
            ...
            Int32U          Pixel;                              // pixels collection


    if (Glcd_Iss==GLCD_ISS_0) {

        pGLCD_CmdCtrl   = (pGLCD_CmdCtrl_t)(GLCD_Cmd_Iss0 + Cmd);
    } else {

        pGLCD_CmdCtrl   = (pGLCD_CmdCtrl_t)(GLCD_Cmd_Iss1 + Cmd);
    }

    if(Write2_DRAM) {

        Ssp0ChipSelect(1);                                      // Select LCD
        if(Cmd != RAMWR) {
            Write2_DRAM = FALSE;
            if(PixelHold & WAITED_PIXEL) {

                Ssp0TranserByte(PixelHold);                     // Flush pixel
            }

            Ssp0ChipSelect(0);                                  // Release CS
        } else {
            ...
            while (Size > 1) {
                ...
                if(!OddPixel) {
                    ...
                    PixelHold |= PIXEL_CONV(Pixel,0);           // Get even pixel
                } else {

                    PixelHold |= PIXEL_CONV(Pixel,1);           // Apply odd pixel

                    Ssp0SendBlock((pInt8U)&PixelHold,3);        // and send together both pixels
                    PixelHold = 0;
                }

                OddPixel ^= 1;                                  // update the even/odd flag

                Size -= 2;                                      // update the counter of the remaining bytes
            }
	...
	if(pGLCD_CmdCtrl->Cmd == GLCD_INV_CMD) {                    // sanitary check of command
	...
	if(Glcd_Iss==GLCD_ISS_0) {
        ...
        switch(Cmd)	{
            ...
            case DATCTR:
                 ...
                 pDATCTR_Data->GS   = 2;                        // 4K colors Type A
                 break;
    ...
    if(pGLCD_CmdCtrl->Param != (Int32U)-1) {
        ...
        Size = pGLCD_CmdCtrl->Param;                            // Command with fixed size of parameters
    }


    Ssp0ChipSelect(1);                                          // Select LCD

    Ssp0TranserByte(pGLCD_CmdCtrl->Cmd);

    if(Cmd == RAMWR) {                                          // enter in write to DRAM mode
        OddPixel    = FALSE;
        PixelHold   = 0;
        Write2_DRAM = TRUE;
    ...
    if(pGLCD_CmdCtrl->Dir == GLCD_READ)   {                     // Send/Receive parameters

        if(pGLCD_CmdCtrl->Pulse) {

            if(Size) {

                *pData++ = Ssp0TranserByte(0xFFFF);
                --Size;
            }
        }
        Ssp0SetWordWidth(8);
    ...
    if(pGLCD_CmdCtrl->Dir == GLCD_READ) {
        ...
        Ssp0ReceiveBlock(pData,Size);
    } else {

        Ssp0SendBlock(pData,Size);
    }
...
GLCD_Status_t  GLCD_PowerUpInit (pInt8U pInitData)
{
    ...
    pDATCTR_Data_t  pDATCTR_Data;
                                                                ...
                                                                // Init SPI and IO
                                                                // Init low level
    Ssp0Init(GLCD_DEF_CLOCK,GLCD_DEF_WIDTH);
    LcdLLInit();
    ...
    if(((Data        & 0xFF) != GLCD_MANF_ID) || (((Data >> 16)& 0xFF) != GLCD_MOD_ID)) {
  	    ...
  	    Glcd_Iss = GLCD_ISS_1;                                  // Iss = 1 don't support RDDID command
    }
    ...
    if(Glcd_Iss == GLCD_ISS_1) {
	    ...
	    GLCD_SendCmd(OSCON,NULL,0);                             //  OSCON (Oscillator ON)

        Data = 0x20 | (0x0UL << 8);                             // Set contrast
        GLCD_SendCmd(VOLCTR,(pInt8U)&Data,0);
    } else {
        Data = 0x30;                                            // Set contrast
    ...
    GLCD_SendCmd(GLCD_DEF_DPL_INV?INVON:INVOFF,NULL,0);         // Display inversion setting
    GLCD_SendCmd(GLCD_DEF_DPL_8_COLOR?IDMON:IDMOFF,NULL,0);     // Display color scheme setting
    ...
    GLCD_SendCmd(DATCTR,(pInt8U)&Data,0);                       // support only 4k type A colored scheme
...
void  GLCD_DrawRectangle (Int32U  x, Int32U  y, Int32U width, Int32U height, Int32U color)
{
    Int32U  temp;
    Int32U  idx;
    ...
    for (idx = 0; idx < height * width; ++idx) {
        GLCD_SendCmd(RAMWR, (pInt8U)&color, 2);
    }
}
    ...
    if (x_dif > y_dif) {
        if (x2 > x1) {
            for (idx = x1; idx <= x2; idx++) {
                y_temp = (y1 * (x2 - x1) + (idx - x1) * (y2 - y1)) / (x2 - x1);
                temp   = idx    | ((idx    + 1) << 8);
                GLCD_SendCmd(CASET, (pInt8U)&temp, 0);
                temp   = y_temp | ((y_temp + 1) << 8);
                GLCD_SendCmd(RASET, (pInt8U)&temp, 0);
                GLCD_SendCmd(RAMWR, (pInt8U)&color, 2);
                GLCD_SendCmd(RAMWR, (pInt8U)&color, 2);
            }
        } else {
            for (idx = x2; idx <= x1; idx++) {
                y_temp = (y2 * (x1 - x2) + (idx - x2) * (y1 - y2)) / (x1 - x2);
                temp   = idx    | ((idx    + 1) << 8);
                GLCD_SendCmd(CASET, (pInt8U)&temp, 0);
                temp   = y_temp | ((y_temp + 1) << 8);
                GLCD_SendCmd(RASET, (pInt8U)&temp, 0);
        ...
        if (y2 > y1) {
            for (idx = y1; idx <= y2; idx++) {
                x_temp = (x1 * (y2 - y1) + (idx - y1) * (x2 - x1)) / (y2 - y1);
                temp   = x_temp | ((x_temp + 1) << 8);
                GLCD_SendCmd(CASET, (pInt8U)&temp, 0);
                temp   = idx    | ((idx    + 1) << 8);
                GLCD_SendCmd(RASET, (pInt8U)&temp, 0);
                GLCD_SendCmd(RAMWR, (pInt8U)&color, 2);
            ...
            for (idx = y2; idx <= y1; idx++) {
                x_temp = (x2 * (y1 - y2) + (idx - y2) * (x1 - x2)) / (y1 - y2);
                temp   = x_temp | ((x_temp + 1) << 8);
                GLCD_SendCmd(CASET, (pInt8U)&temp, 0);
                temp   = idx    | ((idx    + 1) << 8);
                GLCD_SendCmd(RASET, (pInt8U)&temp, 0);
...
int  putchar (int c)
{
    ...
    switch(c) {
        ...
        default:
            ...
            if(GLCD_TextCalcWindow(&xl,&xr,&yu,&yd,&H_Size,&V_Size))  {
	             LCD_SET_WINDOW(xl,xr,yu,yd);	            // set character window X left, Y right
	         ...
	         for(Int32U i = 0; i < V_Size; ++i) {
	             ...
	             pSrc += SrcInc;                            // next line of character
