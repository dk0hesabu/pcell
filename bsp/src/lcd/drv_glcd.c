...
static LcdPixel_t TextColour;
static LcdPixel_t TextBackgndColour;
    ...
    else
    {
      ...
      while(Size > 1)
      {
        ...
        else
        {
          ...
          Ssp0SendBlock((pInt8U)&PixelHold,3);
          PixelHold = 0;
        }
 ...
 *
 *************************************************************************/
GLCD_Status_t LCDPowerUpInit (pInt8U pInitData)
{
Int32U Data;
Int32U i;
pDATCTR_Data_t pDATCTR_Data;
 ...
 *
 *************************************************************************/
void LCDSetFont(pFontType_t pFont, LcdPixel_t Color, LcdPixel_t BackgndColor)
{
  pCurrFont = pFont;
  TextColour = Color;
 ...
 * GLCD_PutHex8Y()
 *
 * Print a unsigned 8-bit hex value at the current cursor position
 *
 * WDH - June 2008
 ************************************************************************/
void LCDPutHex8U( Int8U byte) {
   ...
   upper= ( byte & 0xF0) >> 4;
   lower=   byte & 0x0F;
   putchar( upper > 9 ? 'A' + upper - 10 : '0' + upper );
   putchar( lower > 9 ? 'A' + lower - 10 : '0' + lower );
}

/************************************************************************
 * GLCD_Put16U()
 *
 * Print an unsigned 16-bit hex value at the current cursor position
 * WDH - June 2008
 *
 ************************************************************************/
void LCDPutHex16U( Int16U data ) {
   ...
   int i;
   Int32U shiftMask;
   for ( i= 12, shiftMask= 0xF000; i >= 0; i-= 4, shiftMask >>= 4 ) {
      nybble= ( data & shiftMask ) >> i;
      putchar( nybble > 9 ? 'A' + nybble - 10 : '0' + nybble );
   }
}
 ...
 * GLCD_Put32U()
 *
 * Print an unsigned 32-bit hex value at the current cursor position
 * WDH - June 2008
 *
 ************************************************************************/
void LCDPutHex32U( Int32U data ) {
   ...
   int i;
   Int32U shiftMask;
   for ( i= 28, shiftMask= 0xF0000000; i >= 0; i-= 4, shiftMask >>= 4 ) {
      nybble= ( data & shiftMask ) >> i;
      putchar( nybble > 9 ? 'A' + nybble - 10 : '0' + nybble );
   }
}
 ...
 * GLCD_DrawPixel
 *
 * DrawPixel at position (x,y) in selected Colour
 *
 * WDH - June, 2008
 *
 ************************************************************************/
 ...
 * Returns: nothing
 *
 * Note: good write-up on this algorithm in Wikipedia (search for Bresenham's
 * line algorithm)
 *
 ...
 * LCDDrawCircle()
 *
 * Draws a circle in the specified color at center (x0,y0) with
 * a given radius
 *
...
void LCDDrawCircle( Int32U x0, Int32U y0, Int32U radius, LcdPixel_t Colour ) {
   ...
   int x = 0;
   int y = radius;
   LCDDrawPixel(x0, y0 + radius, Colour);
   LCDDrawPixel(x0, y0 - radius, Colour);
   LCDDrawPixel(x0 + radius, y0, Colour);
   LCDDrawPixel(x0 - radius, y0, Colour);
   while( x < y ) {
      if( f >= 0 ) {
         y--;
      ...
      ddF_x += 2;
      f += ddF_x + 1;
      LCDDrawPixel(x0 + x, y0 + y, Colour);
      LCDDrawPixel(x0 - x, y0 + y, Colour);
      LCDDrawPixel(x0 + x, y0 - y, Colour);
      LCDDrawPixel(x0 - x, y0 - y, Colour);
      LCDDrawPixel(x0 + y, y0 + x, Colour);
      LCDDrawPixel(x0 - y, y0 + x, Colour);
      LCDDrawPixel(x0 + y, y0 - x, Colour);
      LCDDrawPixel(x0 - y, y0 - x, Colour);
   }
 ...
 * LCDDrawFilledCircle()
 *
 * Draws a filled circle in the specified color at center (x0,y0) with
 * a given radius
 *
 * Inputs: x0 = row address (0 .. 131)
 *         y0 = column address (0 .. 131)
 ...
 * color = 12-bit color value rrrrggggbbbb
 *
 * Uses GLCD_DrawCircle() - misses some pixels!
 *
 * Returns: nothing
 *
 ************************************************************************/
 ...
 * LCDDrawRectangle()
 *
 * Draws a rectangle in the specified color from (x0,y0) to (x1,y1)
 *
 * Inputs: x0 = row address (0 .. 131)
 *         y0 = column address (0 .. 131)
 *         x1 = row address (0 .. 131)
 *         y1 = column address (0 .. 131)
 *
 * color = 12-bit color value rrrrggggbbbb
 *
 * Returns: nothing
...
/*************************************************************************
 * LCDDrawFilledRectangle()
 *
 * Draws a filled rectangle in the specified color from (x0,y0) to (x1,y1)
 *
 * Inputs: x0 = row address (0 .. 131)
 *         y0 = column address (0 .. 131)
 *         x1 = row address (0 .. 131)
